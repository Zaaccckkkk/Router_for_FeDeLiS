#!/usr/bin/env python3
"""Train a BERT action scorer for CR-LT using support-alignment rewards.

This trainer is intentionally separate from the WebQSP/CWQ trainer. It consumes
the CR-LT support-alignment tables and optimizes for support-faithful retrieval,
while still reporting the original router-style exact/depth/width metrics on
the comparable support-labeled subset.
"""
...
DEFAULT_PREPROCESSED_DIR = ROUTER_ROOT / "preprocessed" / "crlt"
DEFAULT_OUTPUT_DIR = ROUTER_ROOT / "outputs" / "crlt" / "bert_crlt_support_ranker"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a BERT-based CR-LT action scorer using support-alignment "
            "pointwise regression and pairwise ranking losses."
        )
    )
...
def reasoning_strategy_text(value: Any) -> str:
...
def build_action_text(row: dict[str, Any]) -> str:
    return (
        ...
        f"strategy {reasoning_strategy_text(row.get('reasoning_strategy', []))} "
        f"depth {row['action_depth']} width {row['action_width']} cost {row['action_cost']}"
    )
...
class PointwiseActionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        reward = float(row["support_fbeta"]) - self.reward_cost_weight * (float(row["action_cost"]) / self.max_cost)
...
class PairwiseActionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        row = self.rows[idx]
        left_text = (
            f"task {row['task_type']} strategy {reasoning_strategy_text(row.get('reasoning_strategy', []))} "
            f"depth {row['left_depth']} width {row['left_width']} cost {row['left_cost']}"
        )
        right_text = (
            f"task {row['task_type']} strategy {reasoning_strategy_text(row.get('reasoning_strategy', []))} "
            f"depth {row['right_depth']} width {row['right_width']} cost {row['right_cost']}"
        )
...
def make_query_groups(
    ...
    for query_id, meta in query_meta.items():
        target = None
        if meta.get("support_label_depth") is not None and meta.get("support_label_width") is not None:
            target = (int(meta["support_label_depth"]), int(meta["support_label_width"]))
...
def evaluate_query_actions(
    ...
    predicted_support_fbeta: list[float] = []
    best_support_fbeta: list[float] = []
    baseline_support_fbeta: list[float] = []
    predicted_llm_f1: list[float] = []
    baseline_llm_f1: list[float] = []
    ...
    for query_id in val_query_ids:
        ...
        chosen_score, chosen = scored_actions[0]
        ...
        predicted_support_fbeta.append(float(chosen["support_fbeta"]))
        best_support_fbeta.append(float(meta["best_support_fbeta"]))
        baseline_support_fbeta.append(float(meta["baseline_support_fbeta"]))
        predicted_llm_f1.append(float(chosen["llm_f1"]))
        baseline_llm_f1.append(float(meta["baseline_llm_f1"]))
        ...
        predictions.append(
            {
                ...
                "pred_score": float(chosen_score),
                "pred_support_fbeta": float(chosen["support_fbeta"]),
                "pred_support_recall": float(chosen["support_recall"]),
                "pred_support_precision": float(chosen["support_precision"]),
                "pred_llm_f1": float(chosen["llm_f1"]),
                "best_depth": int(meta["best_depth"]),
                "best_width": int(meta["best_width"]),
                "best_support_fbeta": float(meta["best_support_fbeta"]),
                "baseline_support_fbeta": float(meta["baseline_support_fbeta"]),
                "baseline_llm_f1": float(meta["baseline_llm_f1"]),
                "best_minus_pred_support_fbeta": float(meta["best_support_fbeta"]) - float(chosen["support_fbeta"]),
                "pred_minus_baseline_support_fbeta": float(chosen["support_fbeta"]) - float(meta["baseline_support_fbeta"]),
            }
        )
    ...
    metrics["mean_pred_support_fbeta"] = float(np.mean(predicted_support_fbeta)) if predicted_support_fbeta else 0.0
    metrics["mean_best_support_fbeta"] = float(np.mean(best_support_fbeta)) if best_support_fbeta else 0.0
    metrics["mean_baseline_support_fbeta"] = float(np.mean(baseline_support_fbeta)) if baseline_support_fbeta else 0.0
    metrics["mean_best_minus_pred_support_fbeta"] = float(np.mean([b - p for b, p in zip(best_support_fbeta, predicted_support_fbeta)])) if predicted_support_fbeta else 0.0
    metrics["mean_pred_minus_baseline_support_fbeta"] = float(np.mean([p - b for p, b in zip(predicted_support_fbeta, baseline_support_fbeta)])) if predicted_support_fbeta else 0.0
    metrics["mean_pred_llm_f1"] = float(np.mean(predicted_llm_f1)) if predicted_llm_f1 else 0.0
    metrics["mean_baseline_llm_f1"] = float(np.mean(baseline_llm_f1)) if baseline_llm_f1 else 0.0
...
def main() -> None:
    ...
    for fold_id, (train_query_ids, val_query_ids) in enumerate(splits, start=1):
        ...
        del val_point_loader, val_pair_loader
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            score = float(val_metrics["mean_pred_support_fbeta"])
            ...
            print(
                ...
                f"pair_loss={pair_train_loss:.4f} | support_fbeta={val_metrics['mean_pred_support_fbeta']:.4f} | "
                f"exact_acc={val_metrics['exact_accuracy']:.4f}"
            )
        ...
        print(f"Best epoch: {best_epoch} | best mean_pred_support_fbeta={best_score:.4f}")
    ...
    metric_names = [
        ...
        "mean_best_support_fbeta",
        "mean_baseline_support_fbeta",
    ...
    write_csv(args.output_dir / "bert_crlt_support_ranker_cv_results.csv", fold_results)
    write_csv(args.output_dir / "bert_crlt_support_ranker_cv_summary.csv", summary_rows)
    write_csv(args.output_dir / "bert_crlt_support_ranker_cv_predictions.csv", all_predictions)

    print("\nSaved fold metrics to bert_crlt_support_ranker_cv_results.csv")
    print("Saved summary metrics to bert_crlt_support_ranker_cv_summary.csv")
    print("Saved validation predictions to bert_crlt_support_ranker_cv_predictions.csv")
    print("\nSummary:")
