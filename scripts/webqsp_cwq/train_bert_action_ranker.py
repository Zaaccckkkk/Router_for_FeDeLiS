...
DEFAULT_PREPROCESSED_DIR = ROUTER_ROOT / "preprocessed" / "webqsp_cwq"
DEFAULT_OUTPUT_DIR = ROUTER_ROOT / "outputs" / "webqsp_cwq" / "bert_action_ranker"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=3,
        help=(
            "Number of cross-validation folds. Use 3 for fast model selection and "
            "5 later for confirmation. Must be at least 2."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "weighted_logistic keeps the original ranking loss and uses f1_margin "
            "as a weight; dynamic_margin enforces a margin proportional to f1_margin."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Scale factor gamma for dynamic-margin ranking: target_margin = gamma * f1_margin.",
    )
...
class PairwiseActionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        f1_margin = float(row.get("f1_margin", 0.0))
        weight = 1.0 + f1_margin
        return {
            ...
            "f1_margin": torch.tensor(f1_margin, dtype=torch.float32),
        }
...
def train_pairwise_epoch(
    ...
    for batch in loader:
        ...
        f1_margin = batch["f1_margin"].to(device)
        ...
        if loss_mode == "weighted_logistic":
            ...
            target_margin = dynamic_margin_scale * f1_margin
            pair_loss = F.relu(target_margin - score_gap) * (1.0 + margin_weight * f1_margin)
        else:
...
def main() -> None:
    ...
    if args.n_splits < 2:
        raise ValueError(
            ...
            "If you want a single train/eval run, use a separate held-out split "
            "instead of 1-fold CV."
        )
    set_seed(args.seed)
